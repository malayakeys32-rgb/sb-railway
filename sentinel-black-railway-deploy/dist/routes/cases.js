...
router.get('/', async (req, res) => {
    try {
        const { status, category, severity, location, limit = '10', offset = '0', sortBy = 'createdAt', sortOrder = 'desc', } = req.query;
        // Parse pagination
        const pageLimit = Math.min(Math.max(1, parseInt(limit) || 10), 100); // Max 100
        const pageOffset = Math.max(0, parseInt(offset) || 0);
        // Build filters
        const filters = {};
        if (status && ['OPEN', 'ACTIVE', 'ESCALATED', 'CLOSED'].includes(status)) {
            filters.status = status;
        }
        if (category && ['INFESTATION', 'LANDLORD_NEGLIGENCE', 'HARASSMENT', 'WORKPLACE', 'SAFETY', 'OTHER'].includes(category)) {
        ...
        await (0, audit_1.logAudit)(req.user.userId, 'VIEW', 'CASE', null, { filters, limit: pageLimit, offset: pageOffset });
        return res.json({
            data: cases,
            pagination: {
                limit: pageLimit,
...
router.post('/', async (req, res) => {
    try {
        ...
        await (0, audit_1.logAudit)(req.user.userId, 'CREATE', 'CASE', newCase.id, { title: input.title });
...
router.get('/:id', async (req, res) => {
    try {
        ...
        await (0, audit_1.logAudit)(req.user.userId, 'VIEW', 'CASE', id);
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        await (0, audit_1.logAudit)(req.user.userId, 'UPDATE', 'CASE', id, updateData);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await (0, audit_1.logAudit)(req.user.userId, 'DELETE', 'CASE', id);
...
router.get('/analytics/stats', async (req, res) => {
    try {
        const [totalCases, byStatus, byCategory, bySeverity, recentCreated,] = await Promise.all([
            prisma.case.count(),
            prisma.case.groupBy({
                by: ['status'],
                _count: true,
        ...
        await (0, audit_1.logAudit)(req.user.userId, 'VIEW', 'CASE_ANALYTICS', null);
...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        await (0, audit_1.logAudit)(req.user.userId, 'VIEW', 'CASE', userId, { filter: 'by_user' });
        return res.json({
            data: cases,
            pagination: {
                limit: pageLimit,
